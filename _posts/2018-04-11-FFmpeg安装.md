...
PKG_CONFIG_PATH=$FF_LIB/pkgconfig ./configure --prefix=$FF_BUILD --extra-cflags=-I$FF_INCLUDE --extra-ldflags=-L$FF_LIB --bindir=$FF_BIN --extra-libs=-lpthread --pkg-config-flags="--static" --enable-gpl --enable-nonfree --enable-libfdk-aac --enable-libfreetype --enable-libmp3lame --enable-libopus --enable-libvorbis --enable-libvpx --enable-libx264 --enable-libx265 --enable-pthreads
```
...
解决办法 `export LD_LIBRARY_PATH=$FF_LIB`